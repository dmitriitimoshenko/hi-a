...
import (
	...
	aup "github.com/dmitriitimoshenko/hi-a/google-sheets-accessor/internal/app/bus/handlers/messages/applicationupdateprocessed"
...
func (h *ApplicationUpdateProcessedHandler) Handle(ctx context.Context, message busclient.Message) error {
	var applicationUpdateData *aup.ApplicationUpdatePayload
	if err := json.Unmarshal(message.Value, &applicationUpdateData); err != nil {
		h.logger.Error("[ApplicationUpdateProcessedHandler] failed to unmarshal value from bus message", "err", err, slog.Any("message.Value", message.Value))
		return err
	}
	...
	if applicationUpdateData == nil {
		h.logger.Error("[ApplicationUpdateProcessedHandler] applicationUpdateData is nil", slog.String("bus_key", key))
		return errors.New("failed to extract applicationUpdateData from bus message")
	}
	...
	if err := h.applicationService.UpdateAndSync(ctx, updateApplicationDTO); err != nil {
		...
		return fmt.Errorf("failed to apply application update with bus key [%s]", key)
	}

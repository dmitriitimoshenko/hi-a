...
const (
	...
	STREAM_SAVE_APPLICATION_EMBEDDING   = "STREAM_SAVE_APPLICATION_EMBEDDING"
	STREAM_APPLICATION_UPDATE_PROCESSED = "STREAM_APPLICATION_UPDATE_PROCESSED"
)
...
func (s *BusServer) consumeTopicsHandlers() map[string]func(context.Context, busclient.Message) error {
	...
	if topic := os.Getenv(STREAM_SAVE_APPLICATION_EMBEDDING); topic != "" {
		consumeTopicsHandlers[topic] = s.saveApplicationEmbeddingHandler.Handle
	} else {
		s.logger.Warn("stream is not configured, consumer disabled", slog.String("env", STREAM_SAVE_APPLICATION_EMBEDDING))
	}

	if topic := os.Getenv(STREAM_APPLICATION_UPDATE_PROCESSED); topic != "" {
		consumeTopicsHandlers[topic] = s.applicationUpdateProcessedHandler.Handle
	} else {
		s.logger.Warn("stream is not configured, consumer disabled", slog.String("env", STREAM_APPLICATION_UPDATE_PROCESSED))
	}

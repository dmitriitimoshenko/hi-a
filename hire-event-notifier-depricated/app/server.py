...
logger.info("Starting Hire Event Notifier service...")
...
def _consumer_loop() -> None:
    topics = [
        topic
        for topic in (
            config.KAFKA_TOPIC_HIRE_EVENT,
            config.KAFKA_TOPIC_APPLICATIONS_SYNC_PROCESSED,
        )
        if topic
    ]

    if not topics:
        logger.error("No Kafka topics configured for Hire Event Notifier consumer")
    ...
    try:
        ...
        while not _stop_event.is_set():
            ...
            try:
                ...
                if (
                    config.KAFKA_TOPIC_APPLICATIONS_SYNC_PROCESSED
                    and topic_name == config.KAFKA_TOPIC_APPLICATIONS_SYNC_PROCESSED
                ):
                    kafka_event_handler.handle_application_sync(value)
                elif (
                    config.KAFKA_TOPIC_HIRE_EVENT
                    and topic_name == config.KAFKA_TOPIC_HIRE_EVENT
                ):
                    kafka_event_handler.handle_hire_event(key, value)
                else:
                    logger.warning(
                        "No handler configured for topic '%s', skipping message",
                        topic_name,
                    )
            except Exception:
...
logger.info("Hire Event Notifier service started!")

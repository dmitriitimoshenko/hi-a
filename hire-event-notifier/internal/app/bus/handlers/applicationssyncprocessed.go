...
func NewApplicationUpdateProcessedHandler(logger *slog.Logger, busClient *busclient.Client) *ApplicationUpdateProcessedHandler {
...
func (h *ApplicationUpdateProcessedHandler) Handle(ctx context.Context, message busclient.Message) error {
	...
	if rowID == nil && applicationID == nil {
		...
		h.logger.Error("cannot determine bus key for applications-sync-processed payload", slog.Any("error", err))
	...
	notificationSyncTopic := os.Getenv("STREAM_NOTIFICATION_SYNC")
	if notificationSyncTopic == "" {
		return errors.New("STREAM_NOTIFICATION_SYNC is required")
	}

	if err := h.bus.Publish(ctx, notificationSyncTopic, keyBytes, payloadBytes); err != nil {
		h.logger.Error(

...
const (
	...
	STREAM_HIRE_EVENT                  = "STREAM_HIRE_EVENT"
	STREAM_APPLICATIONS_SYNC_PROCESSED = "STREAM_APPLICATIONS_SYNC_PROCESSED"
)
...
func (s *BusServer) consumeTopicsHandlers() map[string]func(context.Context, busclient.Message) error {
	...
	if topic := os.Getenv(STREAM_HIRE_EVENT); topic != "" {
		consumeTopicsHandlers[topic] = s.hireEventHandler.Handle
	} else {
		s.logger.Warn("stream is not configured, consumer disabled", slog.String("env", STREAM_HIRE_EVENT))
	}

	if topic := os.Getenv(STREAM_APPLICATIONS_SYNC_PROCESSED); topic != "" {
		consumeTopicsHandlers[topic] = s.applicationSyncProcessedHandler.Handle
	} else {
		s.logger.Warn("stream is not configured, consumer disabled", slog.String("env", STREAM_APPLICATIONS_SYNC_PROCESSED))
	}
